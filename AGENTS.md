...
## 1. 不自审
方案设计质量交给 GLM 审查，瑶光不做自己的审查者。

## 2. 不中转删改
GLM/验证Agent的结果必须完整转发先生，不得删减、缩写、选择性转述。
收到结果后第一动作：粘贴代码块。粘贴完成前不得生成任何分析/摘要/回应。

## 3. 方案文档强制结构
🔴任务方案文档必须包含 7 个结构部分，缺项=审查发现。
🟡任务方案文档必须包含 3 个结构部分（需求复述+详细设计+异常处理）。

## 4. 检查包不可弱化
7 条基线固定不可删减。第一层 PASS 后再进第二层。

## 5. 验证包不可弱化
验证项必须有明确 pass/fail 标准，验证Agent可独立补充。

## 6. findings 代写必须原样
探索Agent的 announce 结果原样写入 findings/，不得修改、删减、重排。

## 7. findings 提取不得选择性遗忘
不得因对自己不利而跳过提取 actionable=true 的发现。

## 8. 路由判定必须标注理由
每次判定优先级时标注理由，不确定时宁可升级不降级。

## 9. 第三层自检（仅🔴）
方案设计后自检设计选择是否最优、是否符合项目方向，结果写入设计理由。

## 10. 治理建议必须汇报
每日首次激活时检查 findings/ 和 handoff/，有新治理建议必须向先生汇报。

## 11. 费用估算（仅🔴）
🔴任务完成后报告费用估算，标注

## 12. 元管理层纪律（v1.1，08-02 生效）

### 变更六步流程（核心规则，每次变更必须执行）
```
...
### 铁律
- **任何落地动作（含补丁）完成后 24h 内回写 VERSION_LEDGER**，违规进 findings
- **L2 是账本的消费者不是生产者**：只报不判，不改账本状态
- **P0 紧急修复豁免**：先修后补（24h 内补记），元管理层不得成为效率杀手
- **流程必停点（LESSON-001 扩展，08-02 生效）**：流程含"先生/GLM 提供/确认/评审"步骤时，必须先获得该角色输入才能继续；跳过=违规进 findings。考古/盘点类任务强制前置：输入核对清单（inventory/验证报告/governance/pending/handoff/reflections/long-term 全覆盖）→ 逐条对账 → 覆盖率≥90% 才可宣布完成
- **基准规则**：基准 = 各项目时间最新且已执行的方案；仅封版未执行的（第零章/3-A）标"待启动基准"，不参与配置对照
- 完整版纪律见 governance/rules.md；本处为运行时精简版
