...
- **流程必停点（LESSON-001，08-02 生效）**：流程含"先生提供/确认/回忆"步骤时，必须先询问并收到回复才能继续；跳过=违规进 findings。考古/盘点类任务强制前置：输入核对清单（inventory/验证报告/governance/pending/handoff/reflections/long-term 全覆盖）→ 逐条对账 → 覆盖率≥90% 才可宣布完成
